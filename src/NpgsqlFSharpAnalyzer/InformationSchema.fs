        ...
        let outputColumns =
            [ for column in cols ->
                ...
                if column.TableOID <> 0u then
                    ...
                    {
                        Array = dataType.ClrType.IsArray
                        ColumnAttributeNumber = columnAttributeNumber
        ...
        while row.Read() do
            ...
            | Some oid ->
                ...
                | attnum ->
                    ...
                    let udtName, clrType, isArray =
                        match string row.["col_data_type"] with
                        | "ARRAY" ->
                            let udtName = udtName.TrimStart('_') // remove the marker for the internal pg array
                            let elemType = getTypeMapping udtName
                            udtName, elemType.MakeArrayType(), true
                        | "USER-DEFINED" ->
                            udtName, (if isUdt then typeof<string> else typeof<obj>), false
                        | dataType ->
                            udtName, getTypeMapping(dataType), false
